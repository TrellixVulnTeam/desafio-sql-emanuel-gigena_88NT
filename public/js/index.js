    ...
    socket.on('connect', () => {
        ...
        chatForm.addEventListener('submit', (e) => {
            ...
            const mensaje = {
                ...
                //timeStamp: ts.toLocaleString(), //Esto lo comenté porque ahora lo resuelvo directo con la BD
                msj: inputMsj.value
            }
        ...
        socket.on(PRODMSG, (data) => {
            ...
            if (contadorP == 0) { 
                //Sólo al recibir el primer producto reemplazo subtitulo de "no hay productos" por encabezado de la tabla
        ...
        socket.on(CHATMSG, (data) => {
            ...
            const msg = {
                email: data.email,
                timeStamp: (new Date(data.timeStamp)).toLocaleString(), //Tengo que formatear el timeStamp que viene de la BD antes de imprimirlo
                msj: data.msj
            }
            const item = document.createElement('li');
            chatList.appendChild(item);
            //Compilo el html de un partial de hbs con la data del mensaje de chat
